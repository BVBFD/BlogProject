...
const ClassicHistoryEastern = ({
  ...
  const saveNewWritingData = (event) => {
    ...
    if (editPermission === undefined ? false : editPermission.admin) {
      ...
      const datasUpdate = datasCopy.map((data) => {
        if (data.id === "classicHistoryEastern") {
  ...
  const fixUpdate = (event) => {
    ...
    if (editPermission === undefined ? false : editPermission.admin) {
      ...
      const prevContents = document.querySelectorAll(
        ".classicHistoryEastern_switchBox__2UOPQ"
      );
  ...
  const fixDataTitle =
    keyValue !== "practice" &&
    `
    <h1>${classicHistoryEastern[keyValue - 1]?.type}</h1>
    <h2>${classicHistoryEastern[keyValue - 1]?.title}</h2>
  `;

  const fixImgVid =
    keyValue !== "practice" &&
    `
  ${classicHistoryEastern[keyValue - 1]?.image}
  ${classicHistoryEastern[keyValue - 1]?.video}
`;

  const fixContent =
    keyValue !== "practice" &&
    `
    ${classicHistoryEastern[keyValue - 1]?.contents}
  `;
  ...
  const realTimeFixLinkInputChange = (event) => {
    ...
    let novelUsaEuDataCopy = [...classicHistoryEastern];
    novelUsaEuDataCopy[keyValue - 1].title = event.target.value;
    console.log(novelUsaEuDataCopy);
    setClassicHistoryEastern(novelUsaEuDataCopy);
  };
  ...
  const writeFixFormBtn = (event) => {
    ...
    const datasUpdate = datasCopy.map((data) => {
      if (data.id === "classicHistoryEastern") {
        data.data = classicHistoryEastern;
        return data;
      }
      return data;
    });
        ...
        <div ref={dataBoxForFixRef} className={styles.dataBoxForFix}>
          ...
          <form className={styles.writeForm}>
            <input
              ...
              value={classicHistoryEastern[keyValue - 1]?.title}
            ></input>
            <textarea
              ref={fixTxtAreaRef}
              ...
              value={classicHistoryEastern[keyValue - 1]?.contents}
            ></textarea>

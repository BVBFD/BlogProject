...
const ClassicHistoryWestern = ({
  ...
  let newKey = classicHistoryWestern.length + 1;
  ...
  const saveNewWritingData = (event) => {
    ...
    if (editPermission === undefined ? false : editPermission.admin) {
      const addUpdated = [...classicHistoryWestern];
      addUpdated[newKey - 1] = {
        id: newKey,
        type: classicHistoryWestern[1].type,
        title: newSubTitle,
        contents: `${newTestStr}`,
        image: selectedImg,
        video: selectedVideo,
      };
      setClassicHistoryWestern(addUpdated);

      // datas state 업데이트 하기.
      const datasCopy = [...datas];
      const datasUpdate = datasCopy.map((data) => {
        if (data.id === "classicHistoryWestern") {
      ...
      history.push(
        `/classicHistoryWestern/${classicHistoryWestern.length + 1}`
      );
    } else {
  ...
  const dataRemove = (event) => {
    event.preventDefault();
    if (editPermission === undefined ? false : editPermission.admin) {
      const removeUpdated = [...classicHistoryWestern];
      const filtered = removeUpdated.filter(
        (data) => data.id.toString() !== keyValue
      );
      setClassicHistoryWestern(filtered);
      ...
      writeFormSubTitleInputRef.current.value = "";
      writeFormContentsTextareaRef.current.value = "";
      history.push(`/classicHistoryWestern/${keyValue - 1}`);
    } else {
      alert("블로그 편집 권한이 없습니다. 관리자한테 문의 부탁드립니다");
    }
  };
  ...
  const fixDataTitle =
    keyValue !== "practice" &&
    `
    <h1>${classicHistoryWestern[keyValue - 1]?.type}</h1>
    <h2>${classicHistoryWestern[keyValue - 1]?.title}</h2>
  `;

  const fixImgVid =
    keyValue !== "practice" &&
    `
  ${classicHistoryWestern[keyValue - 1]?.image}
  ${classicHistoryWestern[keyValue - 1]?.video}
`;
  ...
  const writeFixFormBtn = (event) => {
    ...
    const datasUpdate = datasCopy.map((data) => {
      if (data.id === "classicHistoryWestern") {
        data.data = classicHistoryWestern;
        return data;
      }
      return data;
    });
    ...
    <>
      ...
      <li ref={newWritingLiRef} className={styles.newWritingLi}>
        <Route
          ...
          path={`/classicHistoryWestern/practice`}
        >
        ...
        <Link
          ...
          to={`/classicHistoryWestern/practice`}
        >
          <h4>{newKey}.&emsp;</h4>
          <h4>{classicHistoryWestern[0].type}&nbsp;-&nbsp;</h4>
          <h4>{newSubTitle}</h4>
        </Link>
      ...
      {Object.keys(classicHistoryWestern)
        ...
        .map((key) => {
          ...
          const dbImg = classicHistoryWestern[key].image;
          const dbVideo = classicHistoryWestern[key].video;

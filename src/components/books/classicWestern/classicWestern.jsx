...
const ClassicWestern = ({
  ...
  let newKey = classicWestern.length + 1;
  ...
  const saveNewWritingData = (event) => {
    ...
    if (editPermission === undefined ? false : editPermission.admin) {
      const addUpdated = [...classicWestern];
      addUpdated[newKey - 1] = {
        id: newKey,
        type: classicWestern[1].type,
        title: newSubTitle,
        contents: `${newTestStr}`,
        image: selectedImg,
        video: selectedVideo,
      };
      setClassicWestern(addUpdated);
      ...
      history.push(`/classicWestern/${classicWestern.length + 1}`);
    } else {
  ...
  const dataRemove = (event) => {
    ...
    if (editPermission === undefined ? false : editPermission.admin) {
      ...
      writeFormSubTitleInputRef.current.value = "";
      writeFormContentsTextareaRef.current.value = "";
      history.push(`/classicWestern/${keyValue - 1}`);
    } else {
      alert("블로그 편집 권한이 없습니다. 관리자한테 문의 부탁드립니다");
    }
  };
  ...
  const fixUpdate = (event) => {
    ...
    if (editPermission === undefined ? false : editPermission.admin) {
      ...
      const prevContents = document.querySelectorAll(
        ".classicWestern_switchBox__3Lkao"
      );
  ...
  const fixDataTitle =
    keyValue !== "practice" &&
    `
    <h1>${classicWestern[keyValue - 1]?.type}</h1>
    <h2>${classicWestern[keyValue - 1]?.title}</h2>
  `;

  const fixImgVid =
    keyValue !== "practice" &&
    `
  ${classicWestern[keyValue - 1]?.image}
  ${classicWestern[keyValue - 1]?.video}
`;
  ...
  return (
    <>
        ...
        <div ref={dataBoxForFixRef} className={styles.dataBoxForFix}>
          ...
          <form className={styles.writeForm}>
            ...
            <CKEditor
              ...
              data={classicWestern[keyValue - 1]?.contents}
      ...
      <li ref={newWritingLiRef} className={styles.newWritingLi}>
        <Route ref={newWritingLiRouteRef} path={`/classicWestern/practice`}>
          <div className={styles.novelUsaEuBox}>
            <div>
              <h1>{classicWestern[0].type}</h1>
              <h2>{newSubTitle}</h2>
        ...
        <Link
          ...
          to={`/classicWestern/practice`}
        >
          <h4>{newKey}.&emsp;</h4>
          <h4>{classicWestern[0].type}&nbsp;-&nbsp;</h4>
          <h4>{newSubTitle}</h4>
        </Link>
      ...
      {Object.keys(classicWestern)
        ...
        .map((key) => {
          ...
          const dbImg = classicWestern[key].image;
          const dbVideo = classicWestern[key].video;
          let codes = `
              <div>

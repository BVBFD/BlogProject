...
const Vietnam = ({
  ...
  const backFixUpdate = (event) => {
    ...
    const prevContents = document.querySelectorAll(".vietnam_switchBox__3z19Y");
  ...
  const fixDataTitle =
    keyValue !== "practice" &&
    `
    <h1>${vietnams[keyValue - 1]?.type}</h1>
    <h2>${vietnams[keyValue - 1]?.title}</h2>
  `;

  const fixImgVid =
    keyValue !== "practice" &&
    `
  ${vietnams[keyValue - 1]?.image}
  ${vietnams[keyValue - 1]?.video}
`;

  const fixContent =
    keyValue !== "practice" &&
    `
    ${vietnams[keyValue - 1]?.contents}
  `;
        ...
        <div ref={dataBoxForFixRef} className={styles.dataBoxForFix}>
          ...
          <form className={styles.writeForm}>
            <input
              ...
              value={vietnams[keyValue - 1]?.title}
            ></input>
            <textarea
              ref={fixTxtAreaRef}
              ...
              value={vietnams[keyValue - 1]?.contents}
            ></textarea>

...
const China = ({
  ...
  const initialCodes = `
    <div>
      ${chinas[chinas.length - 1].contents}
    </div>`;

  let newKey = chinas.length + 1;
  ...
  const saveNewWritingData = (event) => {
    ...
    if (editPermission === undefined ? false : editPermission.admin) {
      ...
      history.push(`/china/${chinas.length + 1}`);
    } else {
  ...
  const fixDataTitle =
    keyValue !== "practice" &&
    `
    <h1>${chinas[keyValue - 1]?.type}</h1>
    <h2>${chinas[keyValue - 1]?.title}</h2>
  `;

  const fixImgVid =
    keyValue !== "practice" &&
    `
  ${chinas[keyValue - 1]?.image}
  ${chinas[keyValue - 1]?.video}
`;
  ...
  return (
    <>
      ...
      <li ref={newWritingLiRef} className={styles.newWritingLi}>
        <Route ref={newWritingLiRouteRef} path={`/china/practice`}>
        ...
        <Link className={styles.novelUsaEuDataList} to={`/china/practice`}>
          <h4>{newKey}.&emsp;</h4>
          <h4>{chinas[0].type}&nbsp;-&nbsp;</h4>
          <h4>{newSubTitle}</h4>
        </Link>
            ...
            return (
              <li onClick={backFixUpdate}>
                <Link
                  ...
                  to={`/china/${chinas[key].id}`}
                >
                  <h4>{chinas[key].id}.&emsp;</h4>

...
const Chinese = ({
  ...
  const saveNewWritingData = (event) => {
    ...
    if (editPermission === undefined ? false : editPermission.admin) {
      const addUpdated = [...chineses];
      addUpdated[newKey - 1] = {
        id: newKey,
        type: chineses[1].type,
        title: newSubTitle,
        contents: `${newTestStr}`,
        image: selectedImg,
        video: selectedVideo,
      };
      setChineses(addUpdated);
      ...
      history.push(`/chinese/${chineses.length + 1}`);
    } else {
  ...
  const dataRemove = (event) => {
    ...
    if (editPermission === undefined ? false : editPermission.admin) {
      ...
      writeFormSubTitleInputRef.current.value = "";
      writeFormContentsTextareaRef.current.value = "";
      history.push(`/chinese/${keyValue - 1}`);
    } else {
      alert("블로그 편집 권한이 없습니다. 관리자한테 문의 부탁드립니다");
    }
  };
  ...
  const backFixUpdate = (event) => {
    ...
    const prevContents = document.querySelectorAll(".chinese_switchBox__273fq");
  ...
  const fixDataTitle =
    keyValue !== "practice" &&
    `
    <h1>${chineses[keyValue - 1]?.type}</h1>
    <h2>${chineses[keyValue - 1]?.title}</h2>
  `;

  const fixImgVid =
    keyValue !== "practice" &&
    `
  ${chineses[keyValue - 1]?.image}
  ${chineses[keyValue - 1]?.video}
`;

  const fixContent =
    keyValue !== "practice" &&
    `
    ${chineses[keyValue - 1]?.contents}
  `;
  ...
  return (
    <>
        ...
        <div ref={dataBoxForFixRef} className={styles.dataBoxForFix}>
          ...
          <form className={styles.writeForm}>
            <input
              ...
              value={chineses[keyValue - 1]?.title}
            ></input>
            <Editor
            ...
            <textarea
              ...
              value={chineses[keyValue - 1]?.contents}
              readOnly="readOnly"
            ></textarea>
      ...
      <li ref={newWritingLiRef} className={styles.newWritingLi}>
        <Route ref={newWritingLiRouteRef} path={`/chinese/practice`}>
          <div className={styles.novelUsaEuBox}>
            <div>
              <h1>{chineses[0].type}</h1>
              <h2>{newSubTitle}</h2>
        ...
        <Link className={styles.novelUsaEuDataList} to={`/chinese/practice`}>
          <h4>{newKey}.&emsp;</h4>
          <h4>{chineses[0].type}&nbsp;-&nbsp;</h4>
          <h4>{newSubTitle}</h4>
        </Link>
      ...
      {Object.keys(chineses)
        ...
        .map((key) => {
          ...
          const dbImg = chineses[key].image;
          const dbVideo = chineses[key].video;
          let codes = `
              <div>

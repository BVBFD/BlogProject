...
const Reacts = ({
  ...
  const fixDataTitle =
    keyValue !== "practice" &&
    `
    <h1>${reacts[keyValue - 1]?.type}</h1>
    <h2>${reacts[keyValue - 1]?.title}</h2>
  `;

  const fixImgVid =
    keyValue !== "practice" &&
    `
  ${reacts[keyValue - 1]?.image}
  ${reacts[keyValue - 1]?.video}
`;

  const fixContent =
    keyValue !== "practice" &&
    `
    ${reacts[keyValue - 1]?.contents}
  `;
        ...
        <div ref={dataBoxForFixRef} className={styles.dataBoxForFix}>
          ...
          <form className={styles.writeForm}>
            ...
            <textarea
              ...
              value={reacts[keyValue - 1]?.contents}
            ></textarea>

...
const App = ({
  ...
  useEffect(() => {
    const stopSync = dataRepository.syncDatas((vals) => {
      console.log(vals);
      setDatas(vals);
      setNovelUsaEuData(vals[0].data);
      setNovelAsiaData(vals[1].data);
      setClassicWestern(vals[2].data);
      setClassicEastern(vals[3].data);
      setClassicHistoryWestern(vals[4].data);
      setClassicHistoryEastern(vals[5].data);
      setHtmls(vals[6].data);
      setCsss(vals[7].data);
      setJavascripts(vals[8].data);
      setReacts(vals[9].data);
      setNodeJSs(vals[10].data);
      setFijis(vals[11].data);
      setIcelands(vals[12].data);
      setChinas(vals[13].data);
      setVietnams(vals[14].data);
      setEnglishs(vals[15].data);
      setChineses(vals[16].data);
      setVietnameses(vals[17].data);
      setKoreans(vals[18].data);
    });
    return () => stopSync();
  }, [dataRepository]);

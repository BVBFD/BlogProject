...
★성능구현에만 집착하지 말고 깨끗한 코드를 위한 상기의 대원칙을 지키면서 코드할 것!
★객체지향, 깨끗한 코드 지향을 통해서 코드 보수, 유지, 확장에 적합한 코드를 완성할 것!
★JEST, React Test Library를 이용하여 단위 Test 코드 작성 할 것!
★조금 더 깨끗한 코드 구현을 위해 Styled-Components CSS 프레임 워크 적용할 것!
★조금 더 깨끗한 코드 구현을 위해 Redux 상태 관리 툴 대신 React의 useReducer, context API 통해서만 구현해 볼 것!

...
* 두번째 블로그 프로젝트 소회
1. 작년에 진행했던 첫번째 블로그 프로젝트와 달리 Firebase 서버가 아닌, 
2. NodeJS를 통해 직접 서버를 구현함으로써 프론트, 백엔드 풀스택으로 블로그 프로젝트 진행.
3. 첫번째 블로그 프로젝트에 비해서 모든 기능이 빨라지고, 첫번째 블로그 프로젝트에서 실력부족, 성능상 이슈로 구현 못했던 기능들 100% 구현완료
...
2. 성능구현에만 집착하지 말고 깨끗한 코드를 위한 위의 대원칙을 지키면서 코드할 것!

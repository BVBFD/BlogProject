...
### ver 2.9.3 - 2022년 09월 01일
1. 보기 좋은 로딩 과정을 위해 Progressive Circle을 추가함.
2. 필요 없는 모듈인 toast ui editor를 삭제함.

### ver 2.9.5 - 2022년 09월 05일
1. Heroku 무료 서버 배포가 금년 11월 28일부터 중단됨. 그래서 Hoistinger VPS( Virtual Private Server ) 일정 기간 결제 대여하여,
   SSL키를 puttyGen으로 생성. putty를 통해서 싱가포르 원격 VPS로 접속하여 nginx, ubuntu를 이용하여 서버 세팅 및 배포를 하였음. 
   React 사이트도 마찬가지로 방법으로 배포를 함.
2. HTTP로만 일단 배포를 하였음. 추후, 특정 도메인 이름을 대여하여 HTTPS로 업데이트 고민할 예정임 ( 일정 비용이 필요하기 때문... ).
...
#### 두번째 블로그 프로젝트 소회
1. 작년에 진행했던 첫번째 블로그 프로젝트와 달리 Firebase 서버가 아닌, NodeJS를 통해 직접 서버를 구현함으로써 프론트, 백엔드 풀스택으로 블로그 프로젝트 진행.
2. 첫번째 블로그 프로젝트에 비해서 모든 기능이 빨라지고, 첫번째 블로그 프로젝트에서 실력부족, 성능상 이슈로 구현 못했던 기능들 100% 구현완료


#### 두번째 블로그 프로젝트 코드에서 반성할 점
1. 일단 타입스크립트가 아닌 순수 자바스크립트로만 구현하여 유지, 보수에 한계
2. 성능 구현에 급급하여 깨끗한 코드를 위한 아래의 코드 원칙들을 거의 지키지 못함.
   기능 에러, 성능 이슈 해결시 코드 보수 및 확장 할때 시간이 오래걸렸음.
   
    *DRY (Don't Repeat Your Code) <br>
    *KISS (Keep It Simple and Stupid) <br>
    *YAGNI (You Ain't Gonna Need It) <br>


#### 향후 블로그 Ver 3.0 진행시 개선해야 할 점
1. 타입스크립트 도입 및 완벽한 객체 지향 코드로 작성할 것!
2. 성능구현에만 집착하지 말고 깨끗한 코드를 위한 상기의 대원칙을 지키면서 코드할 것!
3. 객체지향, 깨끗한 코드 지향을 통해서 코드 보수, 유지, 확장에 적합한 코드를 완성할 것!
